...
"""
CUDA_VISIBLE_DEVICES=1,2,3,4,5,6 torchrun --nproc_per_node=6 --master_port=12341 -m scripts.train  \
    --config=cosmos_predict2/configs/base/config.py  \
    -- experiment="cospred2_2b_expert_calvin"
"""
data_name = "calvin"
data_name_to_robot_states_dim = {
    ...
    "tcl": 6,      # 6 tcp   + 2 blank
    "calvin": 8,   # 6 joint + 2 gripper
}

n_v_cond, n_v_out = 4 * 0 + 1, 4 * 3  # 0+1+12=13
future_skip = 3
n_a_out = n_v_out
n_v_skip_out = n_v_out // future_skip  # 12/3=4
...
cospred2_2b_expert_calvin = dict(
    defaults=[
        {"override /model": "predict2_v2w_2b_expert_fsdp"},  # modified
        {"override /optimizer": "fusedadamw"},
        {"override /scheduler": "lambdalinear"},
        {"override /ckpt_type": "standard"},
        {"override /dataloader_train": "calvin_train"},  # modified
        "_self_",
    ],
    model=dict(
        config=dict(
            ...
            pipe_config=dict(
                ...
                net=dict(
                    ...
                    action_dof=7,  # calvin: 3 xyz + 3 rot + 1 gripper
            ...
            model_manager_config=dict(
                dit_path="checkpoints/cosmos_predict2/debug/cospred2_2b_expert_calvin_2025-11-04_10-19-20/checkpoints/model/iter_000048000.pt",
            )
        )
    ),
    job=dict(group="debug", name="cospred2_2b_expert_calvin_${now:%Y-%m-%d}_${now:%H-%M-%S}"),
    model_parallel=dict(
        context_parallel_size=1,
    ),
    dataloader_train=dict(
        batch_size=24,  # now:16, ori:20
        num_workers=12,
    ),
    trainer=dict(
    ...
    scheduler=dict(  # better
        cycle_lengths=[30_000, 20_000],  # ori: [20_000, 20_000]
