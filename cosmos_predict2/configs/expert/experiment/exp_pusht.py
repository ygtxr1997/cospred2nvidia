...
"""
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=12341 -m scripts.train  \
    --config=cosmos_predict2/configs/base/config.py  \
    -- experiment="cospred2_2b_expert_pusht"
"""
    ...
    model=dict(
        config=dict(
            ...
            pipe_config=dict(
                ...
                net=dict(
                    action_dim=2*n_a_out,  # (act_dim * horizon)
                    action_dof=2,  # pusht: 2 xy
                    ex_num_latent_frames=n_a_out,
                    ex_dim=1024,
                    ex_adaln_lora_dim=256,
                    extra_robot_states_dim=2*n_v_cond,  # (D*T), (xy) * max_obs
            ...
            #     dit_path="checkpoints/cosmos_predict2/debug/cospred2_2b_expert_pusht_2025-09-29_09-08-00/checkpoints/model/iter_000005000.pt",
            # )
        )
    ),
    job=dict(group="debug", name="cospred2_2b_expert_pusht_${now:%Y-%m-%d}_${now:%H-%M-%S}"),
    model_parallel=dict(
