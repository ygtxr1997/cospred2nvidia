...
class Video2WorldForcePipeline(Video2WorldPipeline):
    ...
    def from_config(
        ...
        if config.ema.enabled:
            log.info(f"Ema enabled. Creating ema model.")
    ...
    def __call__(
        ...
        log.info(F"Starting video generation..., num_sampling_step: {num_sampling_step}")
    ...
    def init_for_online_update(self, max_batches: int = 5, freezing_expert: bool = True,
        ...
        self.dit_ema_fp32_bkp = copy.deepcopy(self.dit_ema).to(
            device="cpu")  # backup fp32 ema model for rollback
        log.info("[INFO] dit_ema_bf16 initialized")

    def reset_for_online_update(self):
