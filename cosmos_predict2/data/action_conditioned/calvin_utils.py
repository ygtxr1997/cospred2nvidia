...
def load_dataset_statistics(train_dataset_dir, val_dataset_dir, transforms):
    ...
    for dataset_type in ["train", "val"]:
        try:
            ...
            statistics = OmegaConf.create(OmegaConf.to_yaml(statistics).replace(
                "calvin_agent.utils.transforms",
                "cosmos_predict2.data.action_conditioned.calvin_utils")
            )
...
##################################################
#### Copied from: mdt.utils.transforms.py ####
...
class RandomShiftsAug(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # 生成单个随机偏移，然后复制到所有帧
        single_shift = torch.randint(0, 2 * self.pad + 1, size=(1, 1, 1, 2), device=x.device, dtype=x.dtype)
        shift = single_shift.repeat(n, 1, 1, 1)
