...
from cosmos_predict2.pipelines.video2world_force import Video2WorldForcePipeline  # modified
from cosmos_predict2.data.action_conditioned.tcl_dataset import TCLImageDataset, TCLMergeDataset  # modified
...
def get_action_sequence(val_dataset, dataset_index=0):  # modified
    data = val_dataset[dataset_index]
    action = data["action"]  # (T,2), in [-1,1]
    force = data["force"]  # (T,6), in [-1,1]
    ...
    return action.numpy(), force.numpy(), agent_pos.numpy(), video.numpy(), t5_text_embeddings
...
def setup_pipeline(args: argparse.Namespace):
    ...
    pipe = Video2WorldForcePipeline.from_config(
        config=config,
        dit_path=dit_path,
...
def process_single_generation(
    ...
    tcl_dataset = TCLMergeDataset(
        ...
        data_roots=[input_path],
        h5_paths=[
            f"{data_parent}/hdf5/{data_task_name}_240p.h5",
        ],
        use_h5=True,
        ...
        future_frame_skip=1,  # ori:1, do not use frame skip for auto-regressive generation
    )
    print("[DEBUG] TCL dataset length:", len(tcl_dataset))
    actions, forces, agent_pos, frames, t5_embeddings = get_action_sequence(tcl_dataset, dataset_index=dataset_index)
    # actions: (T,2), in [-1,1]
    # force: (T,6), in [-1,1]
    ...
    print("[DEBUG] single_generation gt. actions.shape:", actions.shape, "forces.shape:", forces.shape,
          "agent_pos.shape:", agent_pos.shape,
    ...
    data_meta = get_dataset_meta(dit_path)
    ...
    if autoregressive:
        ...
        for i in range(0, len(actions), chunk_size):
            ...
            #     np.arange(frame_start, frame_start + chunk_max_obs),  # v1-1
            #     np.arange(frame_start + chunk_max_obs, frame_start + chunk_max_obs + chunk_size)[::future_skip],  # 1+v2
            # ])
            skipped_indices = get_skipped_indices(frame_start, frame_end, chunk_max_obs, future_frame_skip=future_skip)

            print("[DEBUG] Autoregressive chunk:", i,
                  f"video:[{frame_start},{frame_end}), "
                  f"action:[{action_start},{action_end}), ",
                  f"skipped_indices:{skipped_indices}, "
            )
            ...
            in_frames = cat_multiview_video_with_zeros(
                cond_frames[None, :], sample_n_views, zero_length=chunk_size // future_skip
            ...
            video, out_action, out_force = pipe(
                in_frames,  # T=v1+v2
                actions[action_start: action_end] * 0.,  # H=v2, zero out gt actions
                forces[skipped_indices],  # v1+v2
            ...
            with torch.no_grad():
                ...
                out_force = out_force
                vis_in_force = torch.from_numpy(forces[skipped_indices])

                out_action = tcl_dataset.denorm_action(out_action.cpu().numpy(),
                                                       norm_type=tcl_dataset.norm_action_type,
                                                       meta_data=data_meta["rel_actions"]
                                                       )
                vis_in_action = tcl_dataset.denorm_action(vis_in_action.cpu().numpy(),
                                                          norm_type=tcl_dataset.norm_action_type,
                                                          meta_data=data_meta["rel_actions"]
                                                          )
                out_force = tcl_dataset.norm_state_or_force(out_force.cpu().numpy(),
                                                            norm_type=tcl_dataset.norm_force_type,
                                                            meta_data=data_meta["force_torque"]
                                                            )
                vis_in_force = tcl_dataset.norm_state_or_force(vis_in_force.cpu().numpy(),
                                                               norm_type=tcl_dataset.norm_force_type,
                                                               meta_data=data_meta["force_torque"]
                                                               )

            print("[DEBUG] autoregressive video.shape:", video.shape, "out_action.shape:", out_action.shape,
                  "out_force.shape:", out_force.shape,)
            # video = video[:, :, :-1]  # (1,3,13-1,256,256)
            chunk_video_len_per_view = chunk_max_obs + chunk_size // future_skip  # v1+v2
            ...
            save_action_as_image(out_action[0, :, :3], save_path=f"output/out_action_{i:02d}.png")
            save_action_as_image(vis_in_action[:, :3], save_path=f"output/in_action_{i:02d}.png")
            save_action_as_image(out_force[0, :, :3], save_path=f"output/out_force_{i:02d}.png")
            save_action_as_image(vis_in_force[:, :3], save_path=f"output/in_force_{i:02d}.png")
...
def generate_video(args: argparse.Namespace, pipe: Video2WorldForcePipeline) -> None:
